...
# |> CSV.parse_string()
# |>
Enum.map([["adams-nick","nugent-carol","spouse",nil]], fn [self_slug, relative_slug, relationship, order] ->
  existing_self = Repo.get_by!(Person, slug: self_slug)
  existing_relative = Repo.get_by!(Person, slug: relative_slug)
  Relationship.changeset(%Relationship{}, %{self_id: existing_self.id, relative_id: existing_relative.id, relationship: relationship, order: order})
  end)
